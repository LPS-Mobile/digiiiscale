...
export default function Step5(props) {
    const { pickup, sending, updateOrder } = props
    return (<>
        <div className="detail_wrapper">
            <p style={{ color: "green" }}>Show budtender your badge with id number</p>
            <div className="location_detail">
                <div className="location_detail_wrapper" style={{ border: "none" }}>
                    ...
                    <div>
                        <div>
                            ...
                            </svg></span>{pickup?.user?.fullname}</p>
                            <p>{pickup?.user?.street}</p>
                            {pickup?.user?.phone && <p>ph: {pickup?.user?.phone}</p>}
                        </div>
                    </div>
                </div>
                ...
                <div className="order_detail">
                    <p>Order Details</p>
                    <div>
                        <p>Dispensary Name:{pickup?.dispensary?.name}</p>
                        <p>Picked From: {pickup?.dispensary?.street}{pickup?.dispensary?.city && ","} {pickup?.dispensary?.city}{pickup?.dispensary?.state && ","} {pickup?.dispensary?.state} {pickup?.dispensary?.zip && "-"} {pickup?.dispensary?.zip}</p>
                        {pickup?.dispensary?.phone && <p>ph: {pickup?.dispensary?.phone}</p>}
                    </div>
                    <div className="order_btn">
                        <div className="call_btn">
                            <Button><a href={`tel:${pickup?.phone}`}>Call</a></Button>
                        </div>
                        <div className="deliver_btn">
                            <Button onClick={() => { updateOrder("delivered") }} disabled={sending}>{sending ? "Delivering" : "Deliver"}</Button>
                        </div>
                    </div>
                </div>
            </div>
