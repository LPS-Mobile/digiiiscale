...
export default function Step3(props) {
    const { setStep, pickup, sending, updateOrder } = props
    console.log(pickup)
    return (<>
            ...
            <div className="location_detail">
                <div className="location_detail_wrapper">
                    <div>
                        ...
                        </svg></span>{pickup?.dispensary?.name}</p>
                        <p>{pickup?.dispensary?.street}{pickup?.dispensary?.city && ","} {pickup?.dispensary?.city}{pickup?.dispensary?.state && ","} {pickup?.dispensary?.state} {pickup?.dispensary?.zip && "-"} {pickup?.dispensary?.zip}</p>
                        {pickup?.dispensary?.phone && <p>ph: {pickup?.dispensary?.phone}</p>}
                    </div>
                </div>
                <div className="call_wrapper">
                    <div className="call_btn"><a href={`tel:${pickup?.phone}`}><Button>Call</Button></a></div>
                    <div className="call_cancel_btn"><Button onClick={() => { setStep(2) }}>Cancel</Button></div>
                    <div className="pickup_btn" ><Button onClick={() => { updateOrder("picked") }} sending={sending}>{sending ? "Picking" : "Pickup"}</Button></div>
                </div>
            </div>
        </div>
        <Map route={true} />
